...
def compute_vertex_cost(T, N, K, images, center, min_radius, max_radius):
    ...
    for t in range(T):
        for i in range(N):
            ...
            #print '(from_x, to_x) = ', from_x, to_x
            #print '(from_y, to_y) = ', from_y, to_y
            
            num_pixels = len(coords)
            
                                
            #print num_pixels
            #for temp in range(num_pixels):
             #   print images[t][coords[temp][1], coords[temp][0]]
            for k in range(K):
...
def build_model(T, N, K, w, w_edge, w_e):
    ...
    #connections between left and right frames     

    #Only adding right connections to avoid double adding
    for t in range(T):
        for i in range(N):
            for k in range(K):
                if t > 0 :
    ...
    #(II.)connections between left and right frames                        
    for t in range(T):
        for i in range(N):
            for k in range(K):
                '''
                if t > 0 :
                    #left neighbor (time ) connections
                    for k_prime in range(K):
                        if(ve_index[1][i][k][i][k_prime]!=-1):
                            expr.add(ve[ve_index[1][i][k][j][k_prime-K]], w_e[t][i*K + k][k_prime])
                '''         
                if t < T-1 :

                    #right neighbor (time) connection
                    for k_prime in range(K, K+K):
                        if(ve_index[t][2][i][k][i][k_prime-K]!=-1):
    ...
    #exactly one edge between neighboring columns
    for t in range (T):
        for i in range(N):
            expr2 = LinExpr()
            for k in range(K):
...
def build_model2_fast(T, N, K, w, w_edge, w_e):
    #set model
    m = Model("model_1")
    INF = 9999999999
    # #############################################################################
